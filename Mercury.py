...
class Mercury(Planet.Planet):
	def Name(self):
		return "Mercury"
	def calcMag(self):
		self.M = -0.42 + 5*astro.log10(self.R*self.earthDist) + 0.038*self.i.deg() - 0.000273*pow(self.i.deg(),2) + 0.000002*pow(self.i.deg(),3)
	def loadTerms(self):
		self.Lterms = Lterms
		self.Bterms = Bterms
		self.Rterms = Rterms
	def imageName(self):
		return "Mercury_symbol.png"
